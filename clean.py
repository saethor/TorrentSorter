...
def show_and_season_worker(directory, regex, curr_path, dest_path):
    ...
    for filename in os.listdir(source_season):
        ...
        moveToDest(filename, fil_path, os.path.join(os.path.join(dest_path, name), season))
                    ...
                    if name != source:
                        ...
                        for filename in os.listdir(curr_path):
                            if filename == directory:
                                source_season = os.path.join(curr_path, filename)
                                for fil in os.listdir(source_season):
                                    fil_path = os.path.join(source_season, fil)
                                    moveToDest(fil, fil_path, os.path.join(os.path.join(dest_path, name), season))
                else:
                    show_and_season_worker(directory, get_name_cut_on_season_re, curr_path, dest_path)

            elif re.search(get_single_episode_re, directory, re.IGNORECASE | re.UNICODE):
                show_and_season_worker(directory, get_name_cut_on_episode_re, curr_path, dest_path)
